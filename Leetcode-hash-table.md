...
```c
//因为题目要求时间复杂度为O(n)，所以排序的方法无法实现
//通过哈希表存储数组中的数字 来实现去重+快速访问
//然后遍历原数组  对于遍历到的每个数x 先查看是否有x-1存在(如果有x-1存在，那么这个从x开始的连续序列肯定不是最长子序列，那么跳过这次查找)；如果没有x-1,那么依次查找是否存在x+1,x+2....，同时记录当前连续序列的长度，与最长长度比较并保存较大的那个值。
struct MyNode{
    int data;
    struct MyNode* next;
};
